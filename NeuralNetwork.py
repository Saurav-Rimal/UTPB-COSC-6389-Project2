...
class NeuralNetworkGUI:
    ...
    def create_interface(self):
        self.status_bar = tk.Label(self.root, text="Status: Upload the dataset and modify the values below as required.",
                                   bg="white", fg="black", anchor="w", font=("Arial", 12, "bold"))
        self.status_bar.grid(row=0, column=0, columnspan=2, sticky="we", padx=5, pady=5)
        ...
        self.graph_frame.grid(row=1, column=1, rowspan=3, sticky=tk.NW)
        ...
        if file_path:
            try:
                ...
                self.update_status_bar(
                    f"Status: Dataset imported successfully!"
                )

            except Exception as e:
                self.update_status_bar(f"Status: Could not import dataset:\n{str(e)}")

    def start_training_with_dataset(self):
        if not hasattr(self, 'neuron'):
            self.update_status_bar("Status: Please import the dataset first!")
            return
    ...
    def update_visualization_during_training(self):
        ...
        for i in range(len(layer_positions) - 1):
            max_weightage = np.max(np.abs(self.neuron.synapses[i])) if np.max(np.abs(self.neuron.synapses[i])) != 0 else 1
            for start_idx, node_start in enumerate(layer_positions[i]):
                for end_idx, node_end in enumerate(layer_positions[i + 1]):
                    weightage = self.neuron.synapses[i][start_idx, end_idx]
                    norm_weightageage = abs(weightage) / max_weightage
                    line_width = max(1, int(norm_weightageage * 5))
                    color_intensity = int(norm_weightageage * 255)
                    if weightage >= 0:
    ...
    def reset_network(self):
        ...
        if hasattr(self, 'y'):
            del self.y
        self.update_status_bar("Status: Network Reset. Please import the dataset again.")

    def on_training_complete(self):
        self.update_status_bar("Status: Training complete! Graph Generated.")
